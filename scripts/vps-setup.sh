...
echo "     cd $APP_DIR && sudo -u $APP_USER docker compose -f docker-compose.prod.yml --env-file .env.prod build"
echo "     sudo -u $APP_USER docker compose -f docker-compose.prod.yml --env-file .env.prod up -d"
echo "     # The compose startup gate runs alembic upgrade head before API/Celery start."
