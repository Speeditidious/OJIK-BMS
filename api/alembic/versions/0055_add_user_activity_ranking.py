...
def upgrade() -> None:
    ...
    created_user_sync_events = "user_sync_events" not in existing_tables
    if created_user_sync_events:
        op.create_table(
            "user_sync_events",
            sa.Column(
                "id",
    ...
    if "user_activity_ranking" not in existing_tables:
        op.create_table(
            "user_activity_ranking",
    ...
    op.execute(
        """
        CREATE INDEX IF NOT EXISTS ix_user_activity_ranking_metric_rank
        ON user_activity_ranking (metric, rank)
        """
    )
    ...
    op.execute(
        """
        CREATE INDEX IF NOT EXISTS ix_user_player_stats_user_client_synced_at_desc
